...
def get_file_info_from_mets(sip_directory, file_path_relative_to_sip):
    ...
    if not mets_file:
        ...
        return {}
    job.write_output('Reading METS file', mets_file, 'for reingested file information.')
    ...
    if not entry:
        job.write_output('Could not find', file_path_relative_to_sip, 'in METS.')
...
def main(job, file_uuid=None, file_path='', date='', event_uuid=None, sip_directory='', sip_uuid=None, transfer_uuid=None, use='original', update_use=True):
    ...
    if transfer_uuid:
        ...
        if transfer.type == 'Archivematica AIP':
            job.write_output('updating current location for', file_uuid, 'with', info)
